...
| lightglue_box | `lightglue` | `match` (default: 1 image → its `keypoints`/`descriptors`/`scores`; 2 images → + the matcher's `matches` + `confidence`) / **`stream`** (sliding window: one new frame per call per `session_id`, matched against the last `window` stored frames → per-ref `matches_j`/`confidence_j`) / `reset` (one session, or all) / `list`; stateless models, per-session feature ring (TTL `LIGHTGLUE_SESSION_TTL`); all arrays declared `numpy`; `SUPERPOINT` (default)/`DISK`, `max_keypoints`, `filter_threshold`, `window`, `device` |
